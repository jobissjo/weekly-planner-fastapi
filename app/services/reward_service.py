...
class RewardService:
    ...
    async def get_default_generic_rewards() -> List[dict]:
        return [
            {"title": "Cheat meal", "description": "Order your favorite pizza, burger, or dessert", "is_generic": True},
            {"title": "Play video games", "description": "1 hour of guilt-free video gaming", "is_generic": True},
            {"title": "Watch a movie", "description": "Watch that new movie on your list", "is_generic": True},
            {"title": "Buy a new book", "description": "Get that book you've been wanting to read", "is_generic": True},
            {"title": "Spa or massage", "description": "A relaxing bath or a professional massage", "is_generic": True},
            {"title": "Sleep early", "description": "Go to bed early and get 8+ hours of sleep", "is_generic": True},
        ]
    ...
    async def create_reward(self, user_id: PydanticObjectId, title: str, description: Optional[str]) -> Reward:
        reward = Reward(user_id=user_id, title=title, description=description, is_favorite=False, is_generic=False)
        await reward.insert()
        return reward
    ...
    async def select_favorite_reward(self, user_id: PydanticObjectId, reward_id: str) -> Reward:
        target_reward = await Reward.get(reward_id)
        if not target_reward:
            raise CustomException("Reward not found", 404)

        # Deactivate all other favorite rewards for this user
        user_favorites = await Reward.find(Reward.user_id == user_id, Reward.is_favorite == True).to_list()
        ...
        if target_reward.is_generic:
            ...
            existing_user_copy = await Reward.find(
                ...
                Reward.title == target_reward.title
            ).first_or_none()
    ...
    async def get_favorite_reward(self, user_id: PydanticObjectId) -> Optional[Reward]:
        fav = await Reward.find(Reward.user_id == user_id, Reward.is_favorite == True).first_or_none()
        return fav
